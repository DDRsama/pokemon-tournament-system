...
- `3.3.0-beta` のようなタグ：特定の beta イメージ
- `3.3.0` のようなタグ：特定の安定版イメージ
...
自分の環境でライセンス済みの私有フォントを使う場合は、データディレクトリの `fonts` サブディレクトリ、たとえば Docker 既定の `/data/fonts` に `.ttf / .otf / .ttc / .woff / .woff2` を置けます。システムはファイル名から自動判定し、Web UI で優先的に使用します。PDF レポートはサーバー側で生成され、フォントも埋め込まれます。`/data/fonts` 内の `reportlab` に登録可能なフォントを先に試し、その後に同梱の静的 Noto Sans SC/JP Medium TTF レポート用フォントへフォールバックするため、閲覧端末のローカルフォントには依存しません。私有フォントのライセンス確認はデプロイする利用者の責任であり、公開イメージには同梱しません。
