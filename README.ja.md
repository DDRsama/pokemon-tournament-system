...
現在の beta 版：`3.0.0-beta`

リリースチャンネル：

- `latest`：安定版 Docker イメージ
- `beta`：beta Docker イメージ
- `3.0.0-beta` のようなタグ：特定の beta イメージ
- `3.0.0` のようなタグ：特定の安定版イメージ
...
旧入口 `/admin`、`/overlay`、`/player`、`/player-login` はホームにリダイレクトされます。
...
Docker Compose はデフォルトで以下のディレクトリにデータを永続化します。
...
ゲーム内フォントを参考にした UI フォントを同梱しています。

- `public/shared/fonts/ud-shin-go-sc-r.ttf`

レポート出力時は利用可能な中国語フォントを優先して使用します。Docker イメージには中国語フォントと `reportlab` 実行環境が含まれています。
